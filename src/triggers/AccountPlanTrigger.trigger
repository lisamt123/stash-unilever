trigger AccountPlanTrigger on FS_Account_Plan__c (after insert, after update, before insert, before update) {
    ...
    ObjList=[select id,account_plan__C,approved__C,status__c from FS_Defined_Objective__C where account_plan__C in : Trigger.new];
     ...
     for(FS_Defined_Objective__C Obj : ObjList){
            if(Obj.account_plan__C==AccPlan.id){
