...
<Workflow xmlns="http://soap.sforce.com/2006/04/metadata">
    <alerts>
        ...
        <template>unfiled$public/PACV_Risk_Assessment_Status</template>
    </alerts>
    <alerts>
    ...
    <alerts>
        ...
        <protected>false</protected>
        <recipients>
            <recipient>PACV_PMGroup</recipient>
            <type>group</type>
        </recipients>
        <recipients>
            <recipient>sanket.panda@unilever.com</recipient>
            <type>user</type>
        </recipients>
        <senderType>CurrentUser</senderType>
        <template>unfiled$public/PACV_Risk_Assessment_Template</template>
    </alerts>
    <rules>
        <fullName>PACV Risk Assessment</fullName>
        <actions>
    ...
    <rules>
        <fullName>PACV Risk Assessment Status Completed</fullName>
        <actions>
            <name>Send_an_alert_if_the_status_is_completed_but_no_solution_is_tagged</name>
            <type>Alert</type>
        </actions>
        <active>true</active>
        <formula>AND(ISPICKVAL( PACV_Status__c , &apos;Completed&apos;), ISBLANK(  PACV_Solution_Name__c ) )</formula>
        <triggerType>onCreateOrTriggeringUpdate</triggerType>
    </rules>
    <rules>
        <fullName>PACV_RiskAssesmentEmailRule</fullName>
        <actions>
        ...
        <workflowTimeTriggers>
            <offsetFromField>PACV_Risk_Assessment__c.PACV_Project_End_Date__c</offsetFromField>
